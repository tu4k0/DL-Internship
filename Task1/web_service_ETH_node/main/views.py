...
def index(request):
    ...
    if request.method == 'POST':
            ...
            info = ETHBlockchainInfo(node=provider_url, blockNumber=blockNumber, price=price, protocol=protocol, id_chain=chainId, hashrate=hashrate, mining=mining, maxFee=maxFee)
            info.save()
            ...
            return render(request, 'main/ETHBlockchainInfo.html',
                              {'blockNumber': blockNumber, 'price': price, 'protocol': protocol,
                               'chainId': chainId, 'hashrate': hashrate, 'mining': mining, 'maxFee': maxFee, 'provider_url': provider_url, 'form': form, 'status': status})
    else:
        form = ETHNode()
    return render(request, 'main/ETHBlockchainInfo.html', {'form': form})
